...
// ]
const CountriesList = () => { 
    const {country} = useSelector((state) => state.bot);
    // const countryArray = country.Data?.length > 0 && country.Data;
    // const [countryArray, setCountryArray] = useState([])
    const dispatch = useDispatch();
    const countryId = JSON.parse(localStorage.getItem('countryId'))?.length || null;
    ...
    return (
        <div className="countries">
            ...
            <Link className='countries__link' to='/trashBasket'  >
                <div className={`countries__basket ${countryId ? '' : 'hidden-circle'}`} data-count={countryId}>
